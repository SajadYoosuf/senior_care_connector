...
class _VolunteerMyTasksScreenState extends State<VolunteerMyTasksScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (auth.user != null) {
        context.read<TaskViewModel>().fetchMyTasks(auth.user!.id);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final volunteerId = authViewModel.user?.id ?? '';
  ...
  Widget _buildBody(String volunteerId, AppLocalizations l10n) {
    ...
    if (taskViewModel.isLoading && taskViewModel.myTasks.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    final tasks = taskViewModel.myTasks;
